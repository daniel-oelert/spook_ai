...
| --------- | ----------- | ------------------- |
| **WASM Memory Constraints** | WebAssembly enforces a restrictive approx 2GB memory boundary. Extensive codebase loads inside Pyodide could cause catastrophic out-of-memory (OOM) failures. | Advise/Enforce strict bounds on operations directly touching heavy files. Limit size loaded into MEMFS arrays. |
